...
do
	...
	if [ $percentage -gt $low_battery_percentage ] && [ $notified_low -eq 1 ]; then
		echo "[${DATE}]: Battery charged above low level" | systemd-cat -p info;
		notified_low=0;
	fi

	if [ $percentage -gt $critical_battery_percentage ] && [ $notified_critical -eq 1 ]; then
	...
	if [ $percentage = $low_battery_percentage ] && [ $notified_low -eq 0 ]; then
		...
		echo "[${DATE}]: Notified of low battery" | systemd-cat -p info;
		notified_low=1;
	fi

	if [ "$percentage" = "$critical_battery_percentage" ] && [ $notified_critical -eq 0 ]; then
		echo $notify_critical_command | /bin/bash;
		echo "[${DATE}]: Notified of critical battery" | systemd-cat -p info;
		notified_critical=1;
	fi
	sleep ${update_time};
done
